const adminReports = {
    ...
    renderTable() {
        ...
        const filtered = this.attendanceData.filter(row => {
            ...
            // Normalisasi Tipe (Hapus spasi dan buat huruf besar semua)
        ...
        if (filtered.length === 0) {
            tbody.innerHTML = '<tr><td colspan="10" style="text-align:center; padding:30px; color:#999;">Tidak ada data yang cocok dengan filter ini</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = filtered.map((row, index) => {
            ...
            return `<tr>
                ...
                <td><span style="padding:4px 8px; border-radius:12px; font-size:10px; background:#eee; font-weight:bold;">${String(row.tipe).toUpperCase()}</span></td>
